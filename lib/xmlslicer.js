...
function parseTemplate(template, slicer, itemData) {
    var objData = {};
    ...
    for (var key in template) {
        ...
        if (typeof expr == 'object') {
            objData[key] = parseTemplate.call(this, expr, slicer, itemData);
        }
...
XMLSlicer.prototype.lookForItems = function(currentData) {
    ...
    do {
        ...
        if (match) {
            ...
            this.emit('item', this.index, xmlDoc, itemData);
            
            // if the opdata says we should include it and we have an id
            // then do so
            if (itemData.include && itemData.id) {
                if (this.template) {
                    var objData = parseTemplate.call(xmlDoc, this.template, this, itemData);
                    
                    // trigger the parsed event
                    this.emit('parsed', this.index, objData, itemData);

                    // if the opdata specifies saving the json, then do that now
                    if (itemData.saveJSON) {
                        fs.writeFileSync(
                            basePath + itemData.id + '.json', 
